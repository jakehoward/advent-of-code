...
std::pair<std::vector<int>, std::unordered_map<int, int>> parse_input_ii(const std::string &input) {
    std::vector<int> list_a{};
    ...
    while (sol < input.length()) {
        ...
        list_a.push_back(std::stoi(input.substr(sol, space - sol)));
        b_freq[std::stoi(input.substr(space, eol - space))]++;

        sol = eol + 1;
    }

    return {list_a, b_freq};
}
...
void part_ii(const std::string &input) {
    auto [list_a, b_freqs] = parse_input_ii(input);
